...
def probe_launching_trials(filename):
    ...
    while True:
        ...
        if result[0]:
            ...
            if initial_y_velocity > 100:
                break
    return max(successful_trials, key=lambda item: item[1])
...
def calculate_max_height(velocity_y):
    if velocity_y == 0:
        return velocity_y
    else:
        return velocity_y + calculate_max_height(velocity_y - 1)


if __name__ == "__main__":
    velocity_with_max_y = probe_launching_trials("input.txt")
    print(velocity_with_max_y)
    print(calculate_max_height(velocity_with_max_y[1]))
